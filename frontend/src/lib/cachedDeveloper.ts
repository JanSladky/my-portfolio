...
export type StackItem = {
  ...
  icon:
    | "html" | "css" | "sass" | "js"
    | "react" | "firebase" | "github" | "gitlab"
    | "tailwind" | "nextjs" | "bootstrap" | "vercel";
};
...
function unify(input: any): DeveloperDTO {
  ...
  const work = Array.isArray(a?.work_experiences) ? a.work_experiences : [];
  const edu = Array.isArray(a?.education_experiences) ? a.education_experiences : [];
  const stack = Array.isArray(a?.stack_items) ? a.stack_items : [];
...
async function fetchDeveloperStrict(): Promise<DeveloperDTO> {
  const params =
    "?populate[work_experiences][populate]=*" +
    "&populate[education_experiences][populate]=*" +
    "&populate[stack_items][populate]=*";

  const res = await strapiFetch<{ data: any }>({
    path: "/api/developer" + params,
