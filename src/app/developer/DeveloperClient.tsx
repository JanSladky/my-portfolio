...
export default function DeveloperClient({ initialData }: Props) {
  ...
  return (
    <div className="w-full min-h-screen bg-[#e9f0fb] text-[#1f2937] pt-[120px] pb-[60px] font-sans antialiased">
      <motion.div
        className="w-full max-w-5xl mx-auto px-4 sm:px-6 flex flex-col gap-20"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        <section>
          <div className="space-y-6 text-left">
            <div className="text-left mb-12">
              <h1 className="text-3xl md:text-5xl font-bold text-gray-800 mb-4">
                Infostránka pro případné zaměstnavatele
              </h1>
              <div className="h-1 w-24 bg-blue-400 rounded-full mb-6"></div>
            </div>
            <h3 className="text-xl sm:text-2xl font-semibold text-gray-700 border-b-2 border-blue-500 w-fit pb-1">
              Životopis
            </h3>
          </div>
          ...
          <h4 className="text-xl font-bold text-gray-800 pt-10 pb-4">
            {pageData.acf.cv_title}
          </h4>
          <div className="space-y-5">
            {pageData.acf.cv_work_experience?.map((item: any, index: number) => (
              <div key={index}>
          ...
          <h4 className="text-xl font-bold text-gray-800 pt-10 pb-4">
            {pageData.acf.cv_school_title}
          </h4>
          <div className="space-y-4">
            {pageData.acf.cv_education?.map((item: any, index: number) => (
              <div key={index}>
        ...
        <section className="text-center">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 border-b-4 inline-block border-blue-500 pb-1 mb-2">
            Můj dev stack
          </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 text-center py-10">
            {skills.map(({ src, label }, index) => (
              <div
                key={index}
                className="bg-white shadow-md rounded-xl p-6 hover:shadow-xl transition hover:scale-105"
              >
                <Image
                  src={src}
                  alt={label}
